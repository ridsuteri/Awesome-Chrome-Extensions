...
//apps/drive/frontend/browser/chrome/extensions/save_to_drive/extension/js/contentscriptexpand.js
 ...
 */
goog.module('gdocs.ContentScriptExpand');
...
class ContentScriptExpand {
   ...
   * @private
   */
  getStyleLinkElems_(htmlElem) {
    const styleLinks = [];
    const links = htmlElem.getElementsByTagName('link');
    /*
   ...
   */
  sendExpandedCss_(origHtmlElem) {
    /**
     * A clone of HTML element from the DOM, so we can modify it without
     * affecting the original browser tab.
     * @const {!HTMLElement}
     */
    const htmlElem = /** @type {!HTMLElement} */ (origHtmlElem.cloneNode(true));
    const linkElems = this.getStyleLinkElems_(htmlElem);
   ...
   * @private
   */
  onLoad_(htmlElem, linkEl, e) {
    const xhr = /** @type {!XMLHttpRequest} */ (e.currentTarget);
    if (xhr.status == 200) {
   ...
   * @private
   */
  replaceWithCssStyle_(oldElem, css) {
