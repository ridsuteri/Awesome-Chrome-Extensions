...
//apps/drive/frontend/browser/chrome/extensions/save_to_drive/extension/js/contentscriptscroll.js
 ...
 */
goog.module('gdocs.ContentScriptScroll');
...
const ContentScriptScroll = class {
  ...
  constructor(scrollWindow) {
    ...
    const viewPortSize = this.getViewPortSize_();
     ...
     * @private @const {!Array<!ContentScriptScroll.CornerElemData_>}
     */
    this.fixedElems_ = [];

    /**
     * Elements which are using fixed positioning and are across the bottom.
     * @private @const {!Array<!ContentScriptScroll.EdgeElemData_>}
     */
     ...
     * @private @const {function(!MessageType, !MessageSender, function (*))}
     */
   ...
   * @param {!MessageType} request The message sent from the background page.
   ...
   * @private
   */
  handleMessage_(request, sender, responseFn) {
    switch (request.msg) {
      case 'init':
      ...
      default:
        ...
        this.window_.console.log(
            `ContentScriptScroll.handleMessage invalid message: ${
                request.msg}`);
        break;
    }
  ...
  scrollInit_() {
    ...
    if (this.hasFixedElems_) {
      this.showHideFixedElems_(Corner.TOP_LEFT);
    }
   ...
   */
  scroll_(x, y, cornerPosition, responseFn) {
    ...
    this.waitAnimationFrame_(
        ContentScriptScroll.NUM_ANIMATION_FRAMES, responseFn);
  }
   ...
   * @suppress {deprecated} No base.js so goog.bind() is not available.
   */
  waitAnimationFrame_(numFramesWait, responseFn) {
    if (numFramesWait <= 0) {
   ...
   * @private
   */
  setFixedElems_(visibility) {
    this.fixedElems_.forEach((elementInfo) => {
   ...
   *     createNodeIterator().
   */
  findFixedPosElements_() {
    const lastCapturePartialWidth_ = this.totalWidth_ % this.visibleWidth_;
    const lastCapturePartialHeight_ = this.totalHeight_ % this.visibleHeight_;
    const nodeIterator = this.document_.createNodeIterator(
        this.body_, NodeFilter.SHOW_ELEMENT, null, false);
    let currentNode;
    while (currentNode = nodeIterator.nextNode()) {
      const nodeComputedStyle = this.getComputedStyle_(currentNode);
   ...
   * @param {!HTMLElement} elem The fixed position element.
   ...
   * @private
   */
  computeElemFixedPosition_(
      elem, lastCapturePartialWidth, lastCapturePartialHeight) {
    let rowPosition = '';
    let columnPosition = '';
    let extendsLastRow = false;
    let extendsLastColumn = false;
    ...
    if (this.hasVScrollbar_()) {
      ...
      const offsetTop = this.getOffsetTop(elem);
      if (offsetTop <=
          this.visibleHeight_ - offsetTop - this.getOffsetHeight(elem)) {
        rowPosition = Edge.TOP;
      } else if (offsetTop < this.visibleHeight_) {
        rowPosition = Edge.BOTTOM;
        if (lastCapturePartialHeight != 0) {
          extendsLastRow =
              this.getOffsetHeight(elem) > lastCapturePartialHeight;
        }
      } else {
    ...
    if (this.hasHScrollbar_()) {
      const offsetLeft = this.getOffsetLeft(elem);
      if (offsetLeft <=
          this.visibleWidth_ - offsetLeft - this.getOffsetWidth(elem)) {
        columnPosition = Edge.LEFT;
      } else if (offsetLeft < this.visibleWidth_) {
        columnPosition = Edge.RIGHT;
        if (lastCapturePartialWidth != 0) {
          extendsLastColumn =
              this.getOffsetWidth(elem) > lastCapturePartialWidth;
        }
      } else {
        return;
      }
    ...
    const cornerPosition =
        /** @type {!Corner} */ (`${rowPosition}_${columnPosition}`);
    const cornerData = {
   ...
   * @private
   */
  showHideFixedElems_(capturePosition) {
    this.fixedElems_.forEach((elementInfo) => {
      elementInfo.element.style.visibility =
          (elementInfo.cornerPosition == capturePosition) ? 'visible' :
                                                            'hidden';
    });
  }
   ...
   * @private
   */
  handleSpanningFixedElems_() {
    const widthPos = new Position(
        this.totalWidth_, this.body_.scrollLeft, this.visibleWidth_);
    const heightPos = new Position(
        this.totalHeight_, this.body_.scrollTop, this.visibleHeight_);
    this.checkSpan_(
        widthPos, this.rightPositionElements_, 'offsetWidth', Edge.RIGHT,
        heightPos);
    this.checkSpan_(
        heightPos, this.bottomPositionElements_, 'offsetHeight', Edge.BOTTOM,
        widthPos);
  }
   ...
   * @param {!Position} positionInfo Positional information for the
   *     dimension we are computing.
   * @param {!Array<!ContentScriptScroll.EdgeElemData_>} elements
   *     Array of position and elements.
   * @param {string} offsetWH Either 'offsetWidth' or 'offsetHeight'.
   * @param {!Edge} posRB The edge the element is next to.
   * @param {!Position} otherDimensionPositionInfo Positional
   ...
   * @param {?Edge} position The edge to check.
   * @param {!Position} otherDimensionPositionInfo Positional
   *     information for the opposite dimension we are computing.
   * @return {boolean} Whether this is the corresponding first/last row/column.
   * @private
   */
  isFirstLast_(position, otherDimensionPositionInfo) {
    return position == Edge.LEFT && otherDimensionPositionInfo.isFirst ||
        position == Edge.RIGHT && otherDimensionPositionInfo.isLast ||
        position == Edge.TOP && otherDimensionPositionInfo.isFirst ||
        position == Edge.BOTTOM && otherDimensionPositionInfo.isLast;
  }
   ...
   * @private
   */
  restoreRepositionedElements_() {
    this.repositionedFixedElems_.forEach((data) => {
      const property = data[0];
      const element = data[1];
      const originalValue = data[2];
   ...
   *
   * @return {{width: number, height: number}} The width and height of the view
   *     port.
   * @private
   */
  getViewPortSize_() {
  ...
  /**
   * For testability, create a method which can be overridden by the test
   * framework to get the element's offset width. Returns the width of an
   * element, including borders and padding if any, but not the margins.
   * @param {!HTMLElement} element The element.
 ...
 */
ContentScriptScroll.NUM_ANIMATION_FRAMES = 3;
...
class Position {
   ...
   */
  constructor(totalWH, scrollTL, visibleWH) {
    /**
 ...
 * @private
 */
ContentScriptScroll.CornerElemData_ = {};
 ...
 * @private
 */
ContentScriptScroll.EdgeElemData_ = {};

new ContentScriptScroll(window).init();
